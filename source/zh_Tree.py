...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/zh/Tree/title_kw/' + name + '.json') if os.path.exists('map-tree/zh/Tree/title_kw/' + name + '.json') else None
        with open('map-tree/zh/Tree/title_kw/' + name + '.json', 'w') as outfile:
