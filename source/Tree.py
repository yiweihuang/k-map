        ...
        for first_key in layer_first:
            for item in layer_first[first_key]:
                for title_item in title:
                    if title_item.lower() != item.lower():
                        tup = ((title_item, item),)
...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/en/Tree/title_kw/' + name + '.json') if os.path.exists('map-tree/en/Tree/title_kw/' + name + '.json') else None
        with open('map-tree/en/Tree/title_kw/' + name + '.json', 'w') as outfile:
