...
def get_keyword():
    tfidf_keyword = ['債券評價','年化報酬率','有效年利率','年百分率','債券特性','附買回債券','可轉換公司債','附賣回債券'
	,'指數型債券','債券價格計算','債券殖利率','到期殖利率','YTM','IRR內部報酬率','實質複利率(再投資率)'
    ,'持有期間報酬率','債券價格特性','存續期間']
...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/' + name + '.json') if os.path.exists('map-tree/' + name + '.json') else None
        with open('map-tree/' + name + '.json', 'w') as outfile:
