...
def paser_step_two(type_, week_num, path, layer_one):
    ...
    with open(path, 'r') as fr:
        ...
        new_done = ()
        for item in done_:
            word = fit_word(item[0], tfidf_keyword)
            if word:
                if len(word) == 1:
                    if word[0] != item[1]:
                        temp = (word[0], item[1])
                        if temp not in new_done:
                            new_done = new_done + (temp,)
                else:
                    for w in word:
                        if w != item[1]:
                            temp = (w, item[1])
        ...
        new_arr_tail = []
        for i in new_done:
            new_arr_tail.append(i[1])
            if i[0] not in new_arr_head and i[0] not in new_arr_tail:
                new_arr_head.append(i[0])

        update_done = ()
        a = []
        for elem, next_elem in zip(new_arr_head, new_arr_head[1:] + new_arr_head[ : 1]):
            temp = []
            for i in new_done:
                if (i[0] == elem or i[0] in temp) and i[0] != next_elem:
                    temp.append(i[1])
                    a.append(i)
                    break
        # print(a)
        ...
        a_index = []
        for a_item in a:
            a_index.append(new_done.index(a_item))
        for elem, next_elem in zip(a_index, a_index[1:] + a_index[ : 1]):
            temp = []
            if next_elem != 0:
                for num in range(elem, next_elem, 1):
                    if new_done[num][1] not in temp:
                        temp.append(new_done[num][0])
                        temp.append(new_done[num][1])
                        update_done = update_done + (new_done[num],)
            elif next_elem == 0:
                for num in range(elem, len(new_done), 1):
                    if new_done[num][1] not in temp:
                        temp.append(new_done[num][0])
                        temp.append(new_done[num][1])
                        update_done = update_done + (new_done[num],)
...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/en/Tree/title_kw_new/' + name + '.json') if os.path.exists('map-tree/en/Tree/title_kw_new/' + name + '.json') else None
        with open('map-tree/en/Tree/title_kw_new/' + name + '.json', 'w') as outfile:
            json.dump(root, outfile, indent=4)
    elif type_ == '2':
        os.remove('map-tree/en/Tree/slide_add_book/' + name + '.json') if os.path.exists('map-tree/en/Tree/slide_add_book/' + name + '.json') else None
        with open('map-tree/en/Tree/slide_add_book/' + name + '.json', 'w') as outfile:
            json.dump(root, outfile, indent=4)
