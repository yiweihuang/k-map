...
def paser_step_two(type_, week_num, path, layer_one):
    ...
    with open(path, 'r') as fr:
        ...
        new_arr = []
        for item in done_:
            item_su = re.sub(r'[/()（）()]+', ' ', item[0])
            word = fit_word(item_su, tfidf_keyword)
            if word:
                if len(word) == 1:
                    if word[0] != item[1]:
                        temp = (word[0], item[1])
...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/zh/Tree/title_kw_new/' + name + '.json') if os.path.exists('map-tree/zh/Tree/title_kw_new/' + name + '.json') else None
        with open('map-tree/zh/Tree/title_kw_new/' + name + '.json', 'w') as outfile:
