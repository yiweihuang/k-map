...
def tup_dict_to_tree(type_, name, links):
    ...
    if type_ == '1':
        os.remove('map-tree/en/Tree/title/' + name + '.json') if os.path.exists('map-tree/en/Tree/title/' + name + '.json') else None
        with open('map-tree/en/Tree/title/' + name + '.json', 'w') as outfile:
